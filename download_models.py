...
def download_language_model(source,target):
    ...
    for f in FILENAMES:
        try:
            print(os.path.join(HUGGINGFACE_S3_BASE_URL,model,f).replace("\\", "/"))
            urlretrieve(os.path.join(HUGGINGFACE_S3_BASE_URL,model,f).replace("\\", "/"),
                                        os.path.join(MODEL_PATH,model,f).replace("\\", "/"))
            print("Download complete!")
        except urllib.error.HTTPError:
            print("Error retrieving model from url. Please confirm model exists.")
            os.rmdir(os.path.join("data",model).replace("\\", "/"))
            break
